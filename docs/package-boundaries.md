...
| `entity/` | In-world economic actors and facts: villages, villagers, companies, players, merchants, future financial institutions, ownership/accounting facts, and raw operating records. | Generic market microstructure, product payoff rules, index calculation, or cross-product pricing engines. |
...
- `entity/institution/`: planned financial institution identities, including the default `central_bank_and_securities` provider and future player-created providers. It should own institution identity, ownership, operator permissions, service grants, licenses, and provider metadata, but not market pricing or product payoff logic.
