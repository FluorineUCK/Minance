...
Generic financial product components are documented in `docs/generic-product-components.md`. The current collection is `fund`, `structured`, `future`, and `option`. Current concrete behavior stays in `product/component/fund/`, `product/component/structured/`, and the interim flat `product/component/derivative/` boundary. Java package declarations and imports now align with the physical `product/component/*` paths. Until futures and options are physically split into `product/component/derivative/future/` and `product/component/derivative/option/`, existing flat derivative classes should be treated as migration targets, not as the final owner layout.
