...
| Create index-tracking fund | `FundService.INSTANCE.createIndexTrackingFund(...)` | Commands and future asset-manager logic | Creates fund state and initial holdings |
...
Fund id, manager, strategy tag, cash, shares, current product prices, finance config, and saved fund state.

## Outputs

Fund states, holdings, NAV, share prices, tracking product exposure, and saved NBT state.
